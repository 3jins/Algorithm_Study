...
public class SAMSUNG_12100_Bread {
	...
	public static void move(int[][] m, int num) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (i == 0) {
				boolean[][] merged = new boolean[m.length][m.length];
				mc = rmEmptyPlace(mc, i);
				for (int y = 0; y < n; y++) {
					for (int x = 0; x < n; x++) {
						if (x + dx[i] >= 0 && x + dx[i] < n && y + dy[i] >= 0 && y + dy[i] < n) { // 경계검사
							if (mc[x][y] == mc[x + dx[i]][y + dy[i]] && !merged[x][y] && !merged[x+dx[i]][y+dy[i]]) {
								mc[x + dx[i]][y + dy[i]] += mc[x][y];
								mc[x][y] = 0;
								max = Math.max(max, mc[x + dx[i]][y + dy[i]]);
								merged[x][y] =true;
								merged[x+dx[i]][y+dy[i]] =true;
							}
						}
					}
				}
			...
			if (i == 3) { // 서쪽
				...
				for (int x = 0; x < n; x++) {
					for (int y = 0; y < n; y++) {
						if (x + dx[i] >= 0 && x + dx[i] < n && y + dy[i] >= 0 && y + dy[i] < n) { // 경계검사
							if (mc[x][y] == mc[x + dx[i]][y + dy[i]]&& !merged[x+dx[i]][y+dy[i]]&& !merged[x][y]) {
								mc[x + dx[i]][y + dy[i]] += mc[x][y];
								mc[x][y] = 0;
								max = Math.max(max, mc[x + dx[i]][y + dy[i]]);
								merged[x][y] =true;
